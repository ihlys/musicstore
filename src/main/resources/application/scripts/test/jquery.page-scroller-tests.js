...
describe('Page scroller widget tests.', function() {
  ...
  it('Must load next page of data on scrolled to bottom.', function(done) {
    ...
    $scrollerDiv.pageScroller({scrollPadding: 0});
    $scrollerDiv
      ...
      .animate({ scrollTop: 300 }, { duration: 800, complete: function () {
          const response = '<h1 id="loaded-content3">Loaded page three of content.</h1>';
          ...
          setTimeout(function () {
            ...
            expect($scrollerDiv.find('#loaded-content3')).to.have.lengthOf(1);   
            done();
          }, 1);
        }
      });
